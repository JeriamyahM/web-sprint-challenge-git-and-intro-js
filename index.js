...
function getArtistByIndex(array, ind) {
  return `the artist at index ${ind} is ${array[ind].name}`
}
...
function addArtist(array, namep, yearsp, genrep, nationalityp, biop) {
  let object = {name : namep, years : yearsp, genre : genrep, nationality : nationalityp, bio : biop}
...
function artistInfo(array, name){
  let ind = array.findIndex(arrayname);
  return array[ind].bio
}
...
function artistByCountry(array, nationalityp){
  let boom = []
  for(let i = 0; i < array.length; i++){
    if(array[i].nationality === nationalityp){
      boom.push(array[i].name)
    }
  } 
  return boom
}
